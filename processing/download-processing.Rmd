...
```{r Swtiching experiment_data to long format}

library(tidyr)

# Reshape the data into a long format
experiment_data_long <- experiment_data_widelong %>%
  pivot_longer(
    cols = c(left_response, right_response, left_rating, right_rating), 
    names_to = c(".value", "side"), 
    names_pattern = "(left|right)_(.*)"
  ) %>%
  mutate(side = ifelse(side == "left", "1 left", "2 right"))

# View the first few rows of the reshaped data
head(experiment_data_long)
